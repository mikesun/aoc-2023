...
const Map = struct {
    ...
    fn init(allocator: std.mem.Allocator) !Map {
        return Map{
            .map_ranges = std.ArrayList(MapRange).init(allocator),
        };
    }
    ...
    fn destRanges(
        ...
        input_ranges: std.ArrayList(Range),
    ) !std.ArrayList(Range) {
        var inputs = input_ranges;
        var outputs = std.ArrayList(Range).init(allocator);
        ...
        inputs: while (inputs.popOrNull()) |input| {
            ...
            for (self.map_ranges.items) |mrange| {
                ...
                if (source_start <= input.start) {
                    ...
                    try outputs.append(Range{
                        .start = dest_start + (start - source_start),
                        .end = dest_start + (end - source_start),
                    });

                    // Append any unmapped portion of input
                    if (end < input.end) {
                        try inputs.append(Range{
                            .start = end,
                            .end = input.end,
                        });
                    }
...
fn partTwo(base_allocator: std.mem.Allocator) !void {
    ...
    for (map_list.items) |*m| ranges = try m.destRanges(allocator, ranges);
